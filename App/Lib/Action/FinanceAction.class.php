		...
		switch ($this->type) {
				...
				if($_POST['submit']){
					...
					if($id = $receivables->add($data)){
						if(intval($_POST['check_add_order']) == 1){
							$data_order['name'] = (trim($_POST['order_name']) && (trim($_POST['order_name']) != L('AUTOMATIC_GENERATION')))?trim($_POST['name']):'appoil'.date('Ymd').mt_rand(1000,9999);
			...
			case 'receivingorder' :
				if($_POST['submit']){
					...
					$data['name'] = (trim($_POST['name']) && (trim($_POST['name']) != L('AUTOMATIC_GENERATION')))?trim($_POST['name']):'appoil'.date('Ymd').mt_rand(1000,9999);
