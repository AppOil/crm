...
class WeixinAction extends Action {
	...
	private function responseMsg($data = array("content" => "Welcome to CRM!", 'type'=> "text"))
    {
		...
		if ($this->_post->MsgType == "text") {
			...
			if(!is_array($user) || ($content=="bd" || $content=="绑定")){
				...
				if($content == "xx" || $content == "信息") {
					...
					$data = array('type'=>'text','content'=>'欢迎关注CRM！回复[bd]或[绑定]绑定微信到crm账号,回复[xx]或[消息]查看最新消息(需要您绑定账号)'.$command);
				}
			}
		
		} elseif ($this->_post->MsgType == "image") {
			$data = array('type'=>'text','content'=>'欢迎关注CRM');
